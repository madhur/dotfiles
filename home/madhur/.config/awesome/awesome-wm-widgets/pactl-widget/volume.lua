...
local function worker(user_args)
    ...
    local function update_graphic(widget, force)
        local vol = pactl.get_volume(device)
        if vol ~= nil then
            widget:set_volume_level(vol, force)
        end
    ...
    function volume:inc(s, force)
        pactl.volume_increase(device, s or step)
        update_graphic(volume.widget, force)
    end

    function volume:dec(s, force)
        pactl.volume_decrease(device, s or step)
        update_graphic(volume.widget, force)
    end

    function volume:toggle(force)
