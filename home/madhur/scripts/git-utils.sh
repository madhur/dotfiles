...
# Generate a commit message from the staged diff — now a thin delegate.
#
# The implementation (diff truncation, the system prompt, claude -p invocation,
# output cleanup) moved into the shared homelab lib so the LLM call is
# instrumented (homelab_llm_* metrics, service="claude_cli"). This shim keeps
# the old contract for any caller that still sources git-utils.sh: reads the
# diff from stdin if piped (else `git diff --staged` in cwd), prints the message
# on stdout, prints "" and returns 0 on any failure so callers fall back to
# their own default. See homelab.clients.claude_cli / `homelab-git commit-message`.
generate_llm_commit_message() {
    local homelab_git="/home/madhur/.virtualenvs/python-rsha/bin/homelab-git"

    # No homelab-git available — degrade silently to the empty/fallback path.
    if [ ! -x "$homelab_git" ]; then
        echo ""
        return 0
    fi

    # Forward stdin (piped diff) through; the CLI falls back to `git diff
    # --staged` itself when nothing is piped. --source tags the metric by the
    # repo basename so per-repo LLM spend is attributable in the dashboard.
    local src
    src=$(basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
    "$homelab_git" commit-message --source "$src" 2>/dev/null || echo ""
    return 0
}
